...
const signOutButtun = document.getElementById("signOutButton")
...
signOutButtun.addEventListener("click", (event) => userSignOut())
...
function loadFinanceHistory(currentMonth,  curretDay, currentYear, currentBalance){
    ...
    let dateKey = currentMonth + " " + curretDay + ", " + currentYear
    ...
    if(logMap.has(dateKey)){
        ...
        let logs = logMap.get(dateKey)
        let allExpenseLogs = expenseMap.get(epochDate)
...
function savePressed(){
    ...
    for(let i = 0; i < allExpenseLogs.length; i++){
        ...
        let numericArray = currentEntry.split(' ');
...
function calculateTotal(){
    ...
    for(let i = 0; i < allExpenseLogs.length; i++){
        ...
        let currentEntry = allExpenseLogs[i].textContent;
...
/*
/*import {onAuthStateChanged, signOut} from 'https://www.gstatic.com/firebasejs/9.14.0/firebase-auth.js';
import {auth} from "../scripts/signup.js";
